...
if ( !class_exists( 'Radium_Theme_Demo_Data_Importer' ) ) {
	...
	class Radium_Theme_Demo_Data_Importer extends Radium_Theme_Importer {
		 ...
		 * @var string
		 */
		public $theme_option_name       = 'CARRIE_PANEL'; //set theme options name here (key used to save theme options). Optiontree option name will be set automatically
		...
		public function set_demo_menus(){
			...
			$mailchimp_form_data['markup'] = '<div class="mailchimp-widget-signup-form"><input type="email" id="mc4wp_email" name="EMAIL" placeholder="E-mail address" required /><input type="submit" class="btn alt" value="Subscribe" />
</div>';
			...
			$instagram_data['sb_instagram_at'] = '1522886839.3a81a9f.19eecb59d85e4f0e8e1cac62897ed2b1';
			$instagram_data['sb_instagram_user_id'] = '1522886839';
			$instagram_data['sb_instagram_num'] = '6';
			$instagram_data['sb_instagram_cols'] = '6';
			$instagram_data['sb_instagram_image_padding'] = '0';
		...
		public function import_sliders() {
			...
			if (file_exists(WP_PLUGIN_DIR . '/revslider/revslider.php')) {
				...
				if ( is_dir($dir) ) {
					$hdir = @opendir( $dir );
					if ( $hdir ) {
						echo '<br><b>'.esc_html__('Import Revolution sliders ...', 'carrie').'</b><br>'; flush();
						...
						while (($file = readdir( $hdir ) ) !== false ) {
							...
							if ($response["success"] == false) { 
								echo ' '.esc_html__('Slider was not imported due to error.', 'carrie').'<br>';
							} else {
								echo ' '.esc_html__('Slider imported.', 'carrie').'<br>';
							}
							flush();
						}
						@closedir( $hdir );
				...
				printf(esc_html__('Revolution slider plugin not installed.', 'carrie'), WP_PLUGIN_DIR.'/revslider/revslider.php<br>'); flush();
			}
		}
