...
if(!isset($carrie_theme_options['blog_post_hide_featured_image'])) {
	$carrie_theme_options['blog_post_hide_featured_image'] = false;
}
...
if ( defined('DEMO_MODE') && isset($_GET['post_sidebar_position']) ) {
  $carrie_theme_options['post_sidebar_position'] = $_GET['post_sidebar_position'];
}

if(!isset($carrie_theme_options['post_sidebar_position'])) {
	$carrie_theme_options['post_sidebar_position'] = 'disable';
}
...
if($post_sidebarposition == "0") {
	$post_sidebarposition = $carrie_theme_options['post_sidebar_position'];
}
...
$header_background_image = get_post_meta( get_the_ID(), '_carrie_header_image', true );
...
<div class="container-fluid container-page-item-title<?php echo esc_attr($header_background_class); ?>" data-style="<?php echo esc_attr($header_background_image_style); ?>">
	...
	<div class="page-item-title-single">
	    ...
	    <h1><?php the_title(); ?></h1>
	    <div class="post-date"><i class="fa fa-calendar" aria-hidden="true"></i><?php the_time(get_option( 'date_format' ));  ?></div>

	    <?php if(isset($carrie_theme_options['blog_enable_singlepost_header_info'])&&($carrie_theme_options['blog_enable_singlepost_header_info'])): ?>
...
<?php if ( is_active_sidebar( 'post-sidebar' ) && ( $post_sidebarposition == 'left')) : ?>
		...
		<div class="<?php echo esc_attr($span_class); ?>">
			<div class="blog-post blog-post-single clearfix">
				<?php if(isset($carrie_theme_options['blog_enable_author_info_vertical'])&&($carrie_theme_options['blog_enable_author_info_vertical'])): ?>
				<div class="post-info-vertical">
					...
					<?php comments_popup_link( __( '0 Comments', 'carrie' ), __( '1 Comment', 'carrie' ), __( '% Comments', 'carrie' ) ); ?>
					</div>
					<?php endif; ?>
					<?php if(isset($carrie_theme_options['blog_enable_post_views_counter'])&&($carrie_theme_options['blog_enable_post_views_counter'])): ?>
					<div class="post-info-vertical-views"><?php do_action('carrie_post_views'); // this action called from plugin ?></div>
					<?php endif; ?>

					<?php if(isset($carrie_theme_options['blog_post_show_share'])&&($carrie_theme_options['blog_post_show_share'])): ?>
					<?php if(!isset($post_socialshare_disable) || !$post_socialshare_disable): ?>
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					<div class="post-content-wrapper">
						...
						<div class="post-content">
							<?php
							if ( has_post_thumbnail()&&!in_array($current_post_format, $post_formats_media)&&(!$post_disable_featured_image ) && (!$carrie_theme_options['blog_post_hide_featured_image']) ): // check if the post has a Post Thumbnail assigned to it.
							?>
							<div class="blog-post-thumb">

								<?php the_post_thumbnail('carrie-blog-thumb'); ?>
							...
							carrie_banner_show('single_post_top');
							?>
							...
							<div class="entry-content">
								<?php the_content('<div class="more-link">'.esc_html__( 'Continue reading...', 'carrie' ).'</div>' ); ?>
								<?php
									wp_link_pages( array(
							...
							carrie_banner_show('single_post_bottom');
							?>
				...
				<?php if(isset($carrie_theme_options['blog_enable_post_info_bottom'])&&($carrie_theme_options['blog_enable_post_info_bottom'])): ?>
				<div class="post-info clearfix">
					<?php if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) : ?>
			...
			<?php if(isset($carrie_theme_options['blog_enable_author_info'])&&($carrie_theme_options['blog_enable_author_info'])): ?>
			...
			<?php
			if(isset($carrie_theme_options['blog_post_navigation']) && $carrie_theme_options['blog_post_navigation']) {
				carrie_content_nav( 'nav-below' );
			}
			?>

			<?php if(isset($carrie_theme_options['blog_post_show_related'])&&($carrie_theme_options['blog_post_show_related'])): ?>
