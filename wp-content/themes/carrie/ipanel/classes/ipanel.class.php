    ...
    class IPANEL
    {
        ...
        public function enqueue()
        {
            if ((isset($_GET['page'])) && ('ipanel_' . $this->args['ID'] == $_GET['page'])) {
                $assets_url = CARRIE_IPANEL_URI . 'assets/';
                ...
                wp_localize_script(
                    ...
                    array(
                        ...
                        'reset_confirm_message' => esc_html__("Are you sure you want to reset back to the defaults?", 'carrie'),
                        'confirm_message' => esc_html__("Are you sure?", 'carrie'),
        ...
        function saveOptions($data, $panel_id)
        {
            ...
            $output['message'] = $output['status'] === 'error' ? esc_html__('Error!', 'carrie') : esc_html__('Options Updated.', 'carrie');
            $output['test'] = ($data);
        ...
        function ajaxRequestsHandle()
        {
            ...
            if ($_nonce === false) {
                die(
                    json_encode(
                        array(
                        'status' => 'error',
                        'message' => esc_html__('Error!', 'carrie')
                        )
                    )
                );
            }
            if ($_POST['action2'] == 'save_settings') {
                ...
                if ($_POST['import_type'] == 'code') { // Check if import data is code
                    ...
                    die(
                        json_decode(
                            array(
                            'status' => 'error',
                            'message' => esc_html__("You've not specified any data.", 'carrie')
                            )
                        )
                    );
                }
                ...
                if ($add_option === true) { // Check if add data was successfuly
                    ...
                    $output['message'] = esc_html__('Data has been imported successfully.', 'carrie');
                } else { // and if not
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('An error occured while importing data.', 'carrie');
                }
                ...
                $output['message'] = $delete === false ? esc_html__('Error!', 'carrie') : esc_html__('Options Reset.', 'carrie');
                $this->saveAllOptions();
                ...
                if (@unlink(urldecode($_POST['file_path']))) {
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('The file permanently deleted.', 'carrie');
                } else {
                    $output['status']  = 'error';
                    $output['message'] = esc_html__('ERROR!', 'carrie');
                }
                echo json_encode($output);
                ...
                if (current_user_can('upload_files')) {
                    ...
                    if (array_key_exists('error', $movefile)) {
                        ...
                        if ($is_image) {
                            ...
                            $HTMLRESULTS .= '    '.esc_html__('File URL', 'carrie').': ' . '<code>' . esc_url($movefile['url']) . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File Path', 'carrie').': ' . '<code>' . esc_url($movefile['file']) . '</code><br/><br/>' . "\n";
                            ...
                            $HTMLRESULTS = '    <img class="ipanel-qup-up-img" src="' . esc_url(CARRIE_IPANEL_URI) . 'assets/img/document.png" />' . "\n";
                            $HTMLRESULTS .= '    <div class="ipanel_file_info">' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File URL', 'carrie').': ' . '<code>' . esc_url($movefile['url']) . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File Path', 'carrie').': ' . '<code>' . esc_url($movefile['file']) . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    <a href="#" class="ipanel-remove-qup-file" data-filePath="' . esc_url($movefile['file']) . '">Delete</a>' . "\n";
        ...
        public function addOptions()
        {
            if (get_option($this->args['ID']) === false) {
                if (defined('CARRIE_IPANEL_PLUGIN_USAGE') && CARRIE_IPANEL_PLUGIN_USAGE !== true) {
        ...
        public function downloadExport()
        {
            if (isset($_GET['action']) && $_GET['action'] == 'ipanel-download-export-file') {
                ...
                if ($data === false) {
                    wp_die(esc_html__('Error in download the file.', 'carrie'));
                }
        ...
        public function menuPage()
        {
            ...
            $theme_key_var = 'carrie_license_key_status';
            ...
            if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'registration_complete') {
              update_option($theme_key_var, 'activated');
              delete_option('carrie_license_skip_date');
            }
            ...
            if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'registration_skip') {
              update_option('carrie_license_skip_date', strtotime("+1 day"));
              update_option($theme_key_var, 'activated');
            }

            if(strtotime("now") > get_option( 'carrie_license_skip_date', 0 ) && get_option( 'carrie_license_skip_date', 0 ) > 0) {
              delete_option('carrie_license_skip_date');
              delete_option($theme_key_var);
            }
            ...
            <div class="ipanel-form-stuff" style="margin-top: 20px;">
                ...
                <button class="button button-primary ipanel-save-settings"><?php echo esc_html__('Save All Changes', 'carrie'); ?></button>
               ...
               <a class="button button-primary ipanel-more-themes" target="_blank" href="http://creanncy.com/themes/"><?php echo esc_html__('Premium WordPress Themes', 'carrie'); ?></a>
                <a class="button button-secondary ipanel-more-themes" target="_blank" href="http://support.creanncy.com/"><?php echo esc_html__('Support Center', 'carrie'); ?></a>
                </div>
                <div class="clr"></div>
            </div>
                        ...
                        <td class="ipanel-tabs-list" id="<?php echo esc_attr($this->args["ID"]); ?>-tabs-list">
                            ...
                            <!-- Left Side-->
                                ...
                                <div id="ipanel-tabs">
                                    <ul>
                                        <?php
                                        if($theme_license_status !== 'activated' && (function_exists('carrie_registration_javascript'))):
                                        ?>
                                        <li id="tab-registration"><a href="#" data-target="registration"><?php esc_html_e('Theme Registration', 'carrie'); ?></a></li>
                                        <?php endif; ?>

                                        <?php
                                        // Theme Options Framework show Tabs here. All data inside already correctly escaped and safe.
                                        echo carrie_wp_kses_data($this->generateTabs());
                                        ?>
                        ...
                        <td id="ipanel-fields-container">
                            <div>
                              ...
                              <!-- THEME REGISTRATION FORM -->
                                ...
                                <form id="theme-activation-form" action="themes.php?page=ipanel_CARRIE_PANEL&act=activation" method="POST">
                                <div class="theme-activation-wrapper">
                                  <h1><?php esc_html_e('Welcome to Carrie!', 'carrie'); ?></h1>
                                  <p class="text-large"><?php echo wp_kses_post('Carrie is now installed and ready to use! Please register your purchase to get themes updates notifications,<br>import theme demos and get access to premium dedicated support. We hope you enjoy it!', 'carrie'); ?></p>
                                  <input name="email" type="email" class="activate-theme-email" placeholder="<?php esc_attr_e('Enter your email', 'carrie'); ?>"/><br/><input name="code" type="text" placeholder="<?php esc_attr_e('Enter your theme purchase code', 'carrie'); ?>" class="activate-theme-code"/>
                                  <a href="http://creanncy.com/go/how-to-find-purchase-code/" target="_blank" class="activate-theme-subblink"><?php esc_html_e('How to find your theme purchase code?', 'carrie'); ?></a><br>

                                  <br/><input required class="button button-primary button-hero activate-theme-btn" type="button" value="<?php esc_html_e('Register theme', 'carrie'); ?>"/>
                                  <div class="theme-activation-message"></div>
                                  <p class="text-grey"><br><?php esc_html_e('By using this form you confirm that your purchase code, email (if you provided it) and site url will be saved on our servers for theme license verification purposes. If you provided email address (optional) you will receive important theme updates notifications and news (you can unsubscribe at any time).', 'carrie'); ?></p>
                                ...
                                <form id="ipanel-settings-form">
                                    <?php
                                      ...
                                      echo carrie_wp_kses_data($this->generateFields($this->args['fields']));
                                    ?>
                                </form>
                                <!-- Fields Container -->
            ...
            <div class="ipanel-form-stuff">
                ...
                <button class="button button-secondary ipanel-reset-settings"><i class="ipanel-black-icon trash"></i> <?php echo esc_html__('Reset theme options', 'carrie'); ?></button>

                <?php if ($livePreview) {
                        echo '<button class="button button-secondary ipanel-toggle-preview"><i class="ipanel-black-icon eye"></i> '.esc_html__('Toggle Preview', 'carrie').'</button>';
                    }
                ?>
                </div>
                <div class="clr"></div>
        ...
        public function quickUpload($options = null, $Repeater = array())
        {
            ...
            if (!empty($options['field_options']['btn_text'])) {
                ...
                $btn_text = esc_html__("Choose File", 'carrie');
            }
            ...
            if (filter_var($value['url'], FILTER_VALIDATE_URL) !== false) {
                ...
                if ($is_image) {
                    ...
                    $input .= '    '.esc_html__('File URL', 'carrie').': ' . '<code>' . esc_url($value['url']) . '</code><br/><br/>' . "\n";
                    $input .= '    '.esc_html__('File Path', 'carrie').': ' . '<code>' . esc_url($value['path']) . '</code><br/><br/>' . "\n";
                    $input .= '    <a href="#" class="ipanel-remove-qup-file" data-filePath="' . esc_url($value['path']) . '">'.esc_html__('Delete', 'carrie').'</a>' . "\n";
                    $input .= '    </div>' . "\n";
                    $input .= '    <div class="clr"></div></div>' . "\n";
                } else {
                    // File Box for Other File Types
                    $input .= '    <div class="ipanel-quick-upload-file">' . "\n";
                    $input .= '    <img class="ipanel-qup-up-img" src="' . esc_url(CARRIE_IPANEL_URI) . 'assets/img/document.png" />' . "\n";
                    $input .= '    <div class="ipanel_file_info">' . "\n";
                    $input .= '    '.esc_html__('File URL', 'carrie').': ' . '<code>' . esc_url($value['url']) . '</code><br/><br/>' . "\n";
                    $input .= '    '.esc_html__('File Path', 'carrie').': ' . '<code>' . esc_url($value['path']) . '</code><br/><br/>' . "\n";
        ...
        public function repeater($options = null)
        {
            ...
            if (count($value)) { // If repeater has any item
                foreach ($value as $itemKey => $itemVal) {
                    ...
                    $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'carrie') . '<span class="item_number"> ' . esc_html($i + 1) . '</span>' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf(esc_attr($options['field_options']['each_title']), '<span class="item_number">' . esc_attr($i + 1) . '</span>') . '</h2>';
                ...
                $inputs .= '<i class="ipanel-delete-icon ipanel-delete-repeater-item"></i>';
                $inputs .= '<i class="ipanel-drag-icon ipanel-drag-repeater-item"></i>';
                $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'carrie') . ' 1' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf(esc_attr($options['field_options']['each_title']), '1') . '</h2>';
                $inputs .= '<div class="ipanel-repeater-fields">';
            ...
            $inputs .= '<button class="ipanel-button ipanel-button-white ipanel-repeater-new-item">'.esc_html__("New Item", 'carrie').'</button>';
            $inputs .= '<script type="text/html">';
            $inputs .= '<li class="ipanel-repeater-item" data-repeaterCounter="0">';
            $inputs .= '<i class="ipanel-delete-icon ipanel-delete-repeater-item"></i>';
            $inputs .= '<i class="ipanel-drag-icon ipanel-drag-repeater-item"></i>';
            $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'carrie') . '<span class="item_number"> ' . esc_attr($i + 1) . '</span>' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf(esc_attr($options['field_options']['each_title']), '<span class="item_number">' . esc_attr($i + 1) . '</span>') . '</h2>';
            $inputs .= '<div class="ipanel-repeater-fields">';
        ...
        {
            $button_text  = isset($options['field_options']['button_text']) ? $options['field_options']['button_text'] : esc_html__("Upload", 'carrie');
            $media_title  = isset($options['field_options']['media_title']) ? $options['field_options']['media_title'] : esc_html__("Choose File", 'carrie');
            $media_button = isset($options['field_options']['media_insert_button_text']) ? $options['field_options']['media_insert_button_text'] : esc_html__("Choose File", 'carrie');
            if (isset($options['repeater_item']) && $options['repeater_item'] === true) {
                $option_name = $Repeater['id'] . '[' . $Repeater['pos'] . '][' . $options['id'] . ']';
            } else {
                $option_name = $options['id'];
            ...
            $input .= '    <input class="ipanel-button ipanel-button-white ipanel-media-remove-button" type="button" value="' . esc_html__("Remove", 'carrie') . '"/>';
        ...
        public function export($options = null)
        {
            ...
            if ($export_value === false) {
                $export_value = esc_html__("You need to save options first.", 'carrie');
            }
            $op = '';
            ...
            $op .= '<a href="' . esc_url($url) . '" class="ipanel-button ipanel-button-white ipanel-export-link-btn" target="_blank"> ' . esc_html__('Download Theme Options Backup File', 'carrie') . '</a>';
            $op .= '</div>';
        ...
        public function import($options = null)
        {
            ...
            $op .= '<label><p>' . esc_html__('Paste exported theme options string to import options from backup:', 'carrie') . '</p><textarea class="ipanel-import-text"></textarea></label>';
            $op .= '<br/><br/>' . esc_html__('OR import from file:', 'carrie') . '<br/><br/><label class="ipanel-import-file-label ipanel-button ipanel-button-white"> ' . esc_html__('Choose Theme Options Backup File', 'carrie') . ' <input type="file" name="ipanel-import-file" class="ipanel-import-file"/></label>';
            $op .= '<span class="ipanel-chosen-filename"></span>';
            $op .= '<br/><br/><br/><a class="ipanel-button ipanel-button-white ipanel-import-button">'.esc_html__('Import options', 'carrie').'</a><br/><br/>';
            $op .= '</div>';
            return $this->_fieldSection($options, $op);
        }
