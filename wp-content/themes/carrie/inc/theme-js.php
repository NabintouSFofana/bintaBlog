<?php
	...
	add_action( 'wp_enqueue_scripts', 'carrie_enqueue_dynamic_js', '999' );

	function carrie_enqueue_dynamic_js( ) {
	...
	function carrie_get_js () {
		$carrie_theme_options = carrie_get_theme_options();
		// ===
		ob_start();

        if(isset($carrie_theme_options['custom_js_code']) && trim($carrie_theme_options['custom_js_code'] !== '')):
    ...
    $(document).ready(function() {
        ...
        echo carrie_wp_kses_data($carrie_theme_options['custom_js_code']);  // This variable contains user Custom JS code and can't be escaped with WordPress functions
