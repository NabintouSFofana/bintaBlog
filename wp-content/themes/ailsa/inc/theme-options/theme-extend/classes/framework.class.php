...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( __( 'Success. Imported backup options.', 'ailsa' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( __( 'Default options restored.', 'ailsa' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( __( 'Default options restored for only this section.', 'ailsa' ), 'updated' );
    }
  ...
  public function admin_page() {
    ...
    echo '<div class="cs-framework cs-option-framework">';
      ...
      echo ( $this->settings['ajax_save'] ) ? '<span id="cs-save-ajax">'. __( 'Settings saved.', 'ailsa' ) .'</span>' : '';

      submit_button( __( 'Save', 'ailsa' ), 'primary cs-save', 'save', false, array( 'data-save' => __( 'Saving...', 'ailsa' ) ) );
      submit_button( __( 'Restore', 'ailsa' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );

      if( $this->settings['show_reset_all'] ) {
        submit_button( __( 'Reset All Options', 'ailsa' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
      }

      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. __( 'show all options', 'ailsa' ) .'</a>' : '';
          ...
          foreach( $this->sections as $section ) {
            ...
            if( isset( $section['fields'] ) ) {
              ...
              if ( isset( $section['video_link'] ) ) {
                echo '<a href="'. esc_url($section['video_link']) .'" target="_blank" class="cs-section-video"><i class="fa fa-youtube-play"></i> <span>'. __('How to?', 'ailsa') .'</span></a>';
              }
              echo '</div>';
