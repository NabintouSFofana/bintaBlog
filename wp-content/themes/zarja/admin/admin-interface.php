...
function optionsframework_admin_init() {
		...
		global $of_options_zarja, $pmc_options_machine;
		$pmc_options_machine = new Options_Machine($of_options_zarja);
...
<div class="wrap" id="of_container">
  ...
  <form id="of_form" method="post" action="<?php echo esc_url( ADMIN_DIR . '?page=optionsframework' ) ?>" enctype="multipart/form-data" >
    <div id="header">
      <div class="logo">
        <h2>Zarja - Wordpress Theme</h2> 
      </div>
    ...
    <div id="main">
      <div id="of-nav">
        <ul>
          <?php  echo pmc_security($pmc_options_machine->Menu) ?>
		  <li class="import"><a title="Import" href="#import">Import</a></li>
        </ul>
      </div>
      <div id="content"> <?php pmc_security($pmc_options_machine->Inputs) /* Settings */ ?> 
		<div class="group" id="import"><h2>Import</h2>
				<div class = "pmc-importer-notes">
					...
					<ol>
						...
						<li>To reset your installation we recommend the following plugin: <a href="https://wordpress.org/plugins/wordpress-database-reset/">Wordpress Database Reset</a></li>
						<li>Never run the importer more then once as it will result in extra (double) content.</li>
						<li>If you have any doubts whether you should run importer or not, do not hesitate to <a target="_blank" href = "https://premiumcoding/envato-login">contact us.</a></li>
					</ol>
			<a class="import">Import Demo</a>
		</div>	  
	</div> 	  
      <div id="content"> <?php $pmc_options_machine->Inputs /* Settings */ ?> </div>
      <div class="clear"></div>
    </div>
	<div class="save_bar"> 
    <img style="display:none" src="<?php echo ADMIN_DIR; ?>images/loading-bottom.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
...
function of_admin_head() { 
	...
	$('.colorSelector').each(function(){
		...
		$(this).ColorPicker({
				color: '<?php if(isset($color)) echo esc_attr($color); ?>',
	...
	new AjaxUpload(clickedID, {
		...
		onComplete: function(file, response) {
			...
			// If there was an error
				...
				var buildReturn = '<img class="hide of-option-image" id="image_'+clickedID+'" src="'+response+'"  />';
				$(".upload-error").remove();
	...
	<?php  if(isset($_GET["import"]) && ($_GET["import"] == 'true' || $_GET["import"] == 'false')){   ?>
		$('#of_save').click();
	<?php if(!$_GET["import"] == 'false'){ ?>
...
if(isset($_GET["import"]) && $_GET["import"] == 'start'){   
		...
		$theme_xml = PMC_PATH . 'import/zarja.xml';
...
public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int, $social = false,  $menu = false, $sidebar = false){
	...
	if ($social){
	$slider .= '<label>Font Awesome icon</label>';
	...
	if(!$menu && !$sidebar && !$social){
		...
		if(!empty($val['url'])){
			...
			if($social){
			$slider .= '<img class="of-option-image small" id="image_'.$id.'_'.$order .'" src="'.$val['url'].'" />';}
			else{
